...
**结果：** `fn sq(int x){return x*x;} for(i,5e7){ s += sq(i); }` → recorded=1 compiled=1
...
`fn f(int x){ if(c){return A;} return B; }` 在热循环里被调用,实测 **0.89-0.92×(比解释器慢)**。
...
`clampf`/`reluf` 风格的浮点 helper(`fn f(float x){if(x<0.0){x=0.0;}return x;}`)即被内联 + if-convert(comp=1,exits=1,位精确)。
