...
static void sptir_ensure(SPTIRBuilder *b, int n) {
  if (b->ninst + n <= b->cap)
    return;
  int newcap = b->cap;
  while (newcap < b->ninst + n)
    newcap *= 2;
  if (newcap > SPT_JIT_MAX_IR)
    newcap = SPT_JIT_MAX_IR;
  SPTIRInst *ni = (SPTIRInst *)realloc(b->insts, newcap * sizeof(SPTIRInst));
  if (!ni)
    return; /* out of memory: caller checks */
  b->insts = ni;
  b->cap = newcap;
}
...
int sptir_snapshot(SPTIRBuilder *b, const Instruction *exit_pc) {
  if (b->nsnaps >= SPT_JIT_MAX_SNAPSHOTS)
    return -1;
...
static int64_t ir_const_int(const SPTIRInst *ir) { return ir->aux; }
...
static int64_t spt_ir_idiv(int64_t m, int64_t n) {
  if (n == -1)
    return (int64_t)(0ULL - (uint64_t)m);
  int64_t q = m / n;
  if ((m ^ n) < 0 && m % n != 0)
    q -= 1;
  return q;
...
static int try_const_fold(SPTIRBuilder *b, int idx) {
  ...
  if (a->op == SPTIR_KINT && c->op == SPTIR_KINT) {
    ...
    case SPTIR_SHL:
      ...
      break;
    case SPTIR_SHR:
      r = spt_ir_shiftl(x, (int64_t)(0ULL - (uint64_t)y));
      break;
    case SPTIR_EQ:
      ir->op = (x == y) ? SPTIR_TRUE : SPTIR_FALSE;
      ir->type = SPTT_TRUE;
  ...
  if (sptt_isnum(a->type) && sptt_isnum(c->type) && (a->op == SPTIR_KINT || a->op == SPTIR_KFLT) &&
...
static int try_cse(SPTIRBuilder *b, int idx, int has_table_write) {
  ...
  for (int i = idx - 1; i >= 0; i--) {
    ...
    if (o->op == ir->op && o->type == ir->type && o->op1 == ir->op1 && o->op2 == ir->op2 &&
...
static void guard_dedup(SPTIRBuilder *b) {
  for (int i = 0; i < b->ninst; i++) {
    ...
    for (int j = i - 1; j >= start; j--) {
      ...
      if (o->op == ir->op && o->op1 == ir->op1 && o->op2 == ir->op2 && o->aux == ir->aux &&
          !(o->flags & SPTIRF_DEAD)) {
...
static void mark_used(SPTIRBuilder *b, int ref, uint8_t *used) {
  ...
  /* GUARD_LT/GUARD_LE keep their bound (e.g. an array LEN) in aux as an IR ref,
     ...
     slot at run time. */
  if (ir->op == SPTIR_GUARD_LT || ir->op == SPTIR_GUARD_LE || ir->op == SPTIR_FSELECT)
...
void sptir_dump(const SPTIRBuilder *b, const char *title) {
  ...
  for (int i = 0; i < b->ninst; i++) {
    ...
    if (ir->flags & SPTIRF_GUARD)
      fl[fi++] = 'G';
    if (ir->flags & SPTIRF_SNAP)
      fl[fi++] = 'S';
    if (ir->flags & SPTIRF_DEAD)
      fl[fi++] = 'D';
    if (ir->flags & SPTIRF_PHI)
      fl[fi++] = 'P';
    fl[fi] = 0;
    fprintf(stderr, "  %4d  %-3s %-9s op1=%-4d op2=%-4d aux=%lld  %s\n", i, irtypename(ir->type),
            iropname(ir->op), (int)ir->op1, (int)ir->op2, (long long)ir->aux, fl);
  }
...
static int canon_nop(SPTIRBuilder *b, int ref) {
  int guard = 0;
  while (ref >= 0 && ref < b->ninst && b->insts[ref].op == SPTIR_NOP && guard++ < 256)
    ref = b->insts[ref].op1;
  return ref;
}
...
static void forward_nops(SPTIRBuilder *b) {
  for (int i = 0; i < b->ninst; i++) {
    ...
    if (in->op == SPTIR_NOP)
      continue; /* leave the alias itself */
    if (in->op1 >= 0)
      in->op1 = canon_nop(b, in->op1);
    if (in->op2 >= 0)
      in->op2 = canon_nop(b, in->op2);
    if ((in->op == SPTIR_GUARD_LT || in->op == SPTIR_GUARD_LE) && in->aux >= 0 &&
        in->aux < b->ninst)
      in->aux = canon_nop(b, (int)in->aux);
  }
  for (int s = 0; s <= b->maxslot && s < 256; s++)
    if (b->reg_map[s] >= 0)
      b->reg_map[s] = canon_nop(b, b->reg_map[s]);
}

void sptir_optimize(SPTIRBuilder *b) {
  /* Pass 1: constant folding + algebraic simplification. */
  for (int i = 0; i < b->ninst; i++) {
    if (b->insts[i].flags & SPTIRF_DEAD)
      continue;
    if (try_const_fold(b, i))
      continue;
    try_algebraic(b, i);
  }
