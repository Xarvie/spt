...
- **通用 resume-at-call**(带分支的 callee)——**✅ 已完成(§10.68c)**：exit stub 对 in-callee exit PC 调用 C helper `sptjit_exit_resume` 重建 callee CallInfo（压帧、设 base/savedpc/callstatus），`sptjit_hot_check` 宏更新 `ci/cl/k`，安全网允许有 resume info 的 in-callee exit PC。`proto_is_branch_inlinable` 允许 forward conditional branches + 多 RETURN1。带**循环**的 callee 仍待后续（需循环回边在 callee proto 内的 exit PC 处理）。
