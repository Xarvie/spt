...
- `ctest --test-dir build -j4`：290 项
- `scripts/jit_difftest.sh`：76 个 kernel 差分（JIT-off 全为 0，即每个 kernel 都真正编译出 trace）
- `scripts/jit_difftest_all.sh`：全量 `test/**/*.spt` 差分（287 一致，跳过 `b4_yield_return`）
...
注：**纯直线叶函数调用现在会内联**（见阶段 1），不再中止。**嵌套循环**：内层循环各自从自己的头编译
...
- **方法调用 / OP_SELF + CALL(后续)**:需调用帧 + 字段访问(GETFIELD 已铺垫字段读)。多数最终想要 C 调用机制或更多内联哈希工作。**C 调用机制已于 §10.41/§10.42 落地**,方法调用现主要缺调用帧拼装。
