...
- `ctest --test-dir build -j4`：308 项
- `scripts/jit_difftest.sh`：94 个 kernel 差分（JIT-off 全为 0，即每个 kernel 都真正编译出 trace）
- `scripts/jit_difftest_all.sh`：全量 `test/**/*.spt` 差分（305 一致，跳过 `b4_yield_return`）
