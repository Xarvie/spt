...
> 配套文档：实现细节见 `JIT_DEV_NOTES.md`（§10.1–§10.62）。
...
最近交付：**§10.62 SLEN/SBYTE resume-at-SELF**（落地 §10.61 预告的 string.len/byte 长串侧出潜在 bug——守卫快照恢复到 SELF 而非 CALL，同修单循环与嵌套；曾探索的 R[B] 物化被证多余[no-op]且有害[引入非确定崩溃]、已移除）+ **lcode.c luaK_self obj[key]() 段错误修复**（`a[3]()`/`m[k]()` 的非 VKSTR 键读联合体垃圾 `key->u.strval` → `SEGV in luaK_self`；条件加 `key->k == VKSTR &&` 短路落 move+gettable，与 JIT 嵌套正交），全门槛绿、no-jit=0、ASan 干净。前一阶 **§10.61 math.floor/ceil**（复用死桩 SPTIR_TOINT；roundsd + 转换往返值域守卫，越界/NaN 守卫 resume-at-SELF；记录时值域观测止颠簸）、**§10.60 math.abs**（int select / float FMATH(fabs)，零新 IR）同绿。
进行中：无。
**接手者请先读仓库根 `HANDOFF.md`**（构建/门槛命令、后续优先级路线、纪律与坑），再看本文件与 `JIT_DEV_NOTES.md` 的细节。
后续优先级：运行期守卫失败黑名单（根治颠簸，触及 trace 生命周期；SLEN/SBYTE 长串侧出已于 §10.62 改为 resume-at-SELF）→ string.sub（GC）→ 嵌套内联多帧 / 通用多写。
