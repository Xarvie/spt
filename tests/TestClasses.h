...
inline void registerClasses(Test &runner) {
  runner.addTest("Simulate Class with Metatable Explicit",
                 R"(
        map<any, any> M = {};
        M["__index"] = M;

        // 这里的 self 对应 Slot 1，name 对应 Slot 2。Slot 0 被隐式预留且为 nil
        void MyClass_ctor(any self, string name) {
            self.name = name;
        }
        M.ctor = MyClass_ctor;

        // 这里的 meta 对应 Slot 1，name 对应 Slot 2
        map MyClass_create(any meta, string name) {
            map<any, any> obj = {};
            setmetatable(obj, meta);

            // 显式地把 obj 传给 Slot 1 (self)，把 name 传给 Slot 2
            meta.ctor(obj, name);
            return obj;
        }
        M.create = MyClass_create;

        // 显式地把 M 传给 Slot 1 (meta)，把 "VirtualClass" 传给 Slot 2
        map p = M.create(M, "VirtualClass");
        print(p.name);
    )",
                 "VirtualClass");
