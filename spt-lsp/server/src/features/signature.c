...
cJSON *feature_signature_help(const Document *d, LspPos pos, Workspace *ws) {
  ...
  if (open >= 0) {
    ...
    if (name_tok) {
      ...
      if (nl >= sizeof name)
        nl = sizeof name - 1;
      memcpy(name, name_tok->lexeme, nl);
      name[nl] = '\0';
      ...
      if (!fn && ws) {
        ...
        if (sem_import_binding_path(u, name, mod_path, sizeof mod_path)) {
          char tgt_uri[4096];
          if (workspace_resolve_module(ws, d->uri ? d->uri : "", mod_path, tgt_uri,
                                       sizeof tgt_uri)) {
            char tgt_path[4096];
      ...
      if (fn) {
        ...
        if (fn->u.func_decl.is_variadic)
          sappend(label, sizeof label, &lp, "%s...", ps->count ? ", " : "");
