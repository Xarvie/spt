...
cJSON *feature_document_link(const Document *d, const char *uri, Workspace *ws) {
  ...
  for (int i = 0; i < u->token_count; i++) {
    if (u->tokens[i].kind != TOK_STRING_LITERAL)
      continue;
    /* 前一个 token 应为 TOK_FROM（import ... from "mod"）。 */
    if (i == 0 || u->tokens[i - 1].kind != TOK_FROM)
      continue;
    ...
    if (tl < 2)
      continue; /* 至少一对引号 */
    size_t pl = tl - 2;
    if (pl >= sizeof mod_path)
      pl = sizeof mod_path - 1;
    memcpy(mod_path, t->lexeme + 1, pl);
    mod_path[pl] = '\0';

    /* 解析目标文件 URI。 */
    char tgt_uri[4096];
    if (!workspace_resolve_module(ws, uri, mod_path, tgt_uri, sizeof tgt_uri))
      continue;
