...
const char *const SPT_TOKEN_TYPES[] = {"keyword",  "type",      "class",     "function",
                                       "variable", "parameter", "property",  "string",
                                       "number",   "operator",  "namespace", "comment"};
...
static void ns_add(NameSet *s, const char *nm) {
  ...
  if (dbg) {
    fprintf(dbg, "ns_add: n=%d cap=%d nm=%s\n", s->n, s->cap, nm);
    fflush(dbg);
  }
  ...
  if (s->n >= s->cap) {
    ...
    if (dbg) {
      fprintf(dbg, "ns_add: realloc cap %d->%d\n", s->cap, newcap);
      fflush(dbg);
    }
    s->a = realloc(s->a, sizeof(char *) * (size_t)newcap);
    if (dbg) {
      fprintf(dbg, "ns_add: realloc returned %p\n", (void *)s->a);
      fflush(dbg);
    }
    s->cap = newcap;
  }
  s->a[s->n] = nm;
  if (dbg) {
    fprintf(dbg, "ns_add: set a[%d]=%s\n", s->n, nm);
    fflush(dbg);
  }
  s->n++;
  if (dbg) {
    fprintf(dbg, "ns_add: done n=%d\n", s->n);
    fflush(dbg);
    fclose(dbg);
  }
}
...
static int ns_has(const NameSet *s, const char *nm, size_t len) {
  for (int i = 0; i < s->n; i++)
    if (strlen(s->a[i]) == len && memcmp(s->a[i], nm, len) == 0)
      return 1;
  return 0;
}
...
static void gather(const AstNode *root, NameSet *funcs, NameSet *classes) {
  ...
  for (int i = 0; i < st->count; i++) {
    ...
    if (s->type == NODE_FUNCTION_DECL) {
      ...
      if (dbg) {
        fprintf(dbg, "gather: func name=%p (%s)\n", (void *)nm, nm ? nm : "(null)");
        fflush(dbg);
      }
      ns_add(funcs, nm);
...
cJSON *feature_semantic_tokens_full(const Document *d) {
  FILE *dbg = spt_open_log();
  if (dbg) {
    fprintf(dbg, "semtok: start, text_len=%zu line_count=%d\n", d->text_len, d->line_count);
    fflush(dbg);
  }

  SptLspUnit *u = spt_lsp_parse(d->text, d->text_len);
  if (dbg) {
    fprintf(dbg, "semtok: parsed, u=%p root=%p tokens=%d\n", (void *)u, u ? (void *)u->root : NULL,
            u ? u->token_count : 0);
    fflush(dbg);
  }
  ...
  if (u && u->root) {
    ...
    if (dbg) {
      fprintf(dbg, "semtok: gathered, funcs=%d classes=%d\n", funcs.n, classes.n);
      fflush(dbg);
    }
    ...
    for (int i = 0; i < u->token_count; i++) {
      ...
      if (t->kind != TOK_IDENTIFIER)
        continue;
      int member =
          (i > 0 && (u->tokens[i - 1].kind == TOK_DOT || u->tokens[i - 1].kind == TOK_COLON));
      int type;
      if (member)
        type = TT_PROPERTY;
      else if (ns_has(&funcs, t->lexeme, (size_t)t->length))
        type = TT_FUNCTION;
      else if (ns_has(&classes, t->lexeme, (size_t)t->length))
        type = TT_CLASS;
      else
        type = TT_VARIABLE;

      if (dbg && i < 20) {
        fprintf(dbg, "semtok: tok[%d] lexeme=%s line=%d col=%d len=%d\n", i,
                t->lexeme ? t->lexeme : "(null)", t->line, t->column, t->length);
        fflush(dbg);
      }
    ...
    free(classes.a);
    if (dbg) {
      fprintf(dbg, "semtok: loop done\n");
      fflush(dbg);
    }
  }
  spt_lsp_unit_free(u);
  if (dbg) {
    fprintf(dbg, "semtok: unit freed\n");
    fflush(dbg);
  }
  cJSON *res = cJSON_CreateObject();
  cJSON_AddItemToObject(res, "data", data);
  if (dbg) {
    fprintf(dbg, "semtok: done\n");
    fflush(dbg);
    fclose(dbg);
  }
  return res;
