...
int main(void) {
  ...
  /* ---- resolve_module_path 单元测试 ---- */
  {
    ...
    snprintf(expect, sizeof expect, "%s/b.spt", dir);
#endif
    CHECK(resolve_module_path(from_path, "b", got, sizeof got) == 1,
          "resolve_module_path finds b.spt");
    CHECK(strcmp(got, expect) == 0, "resolve_module_path returns script_dir/b.spt");
    CHECK(resolve_module_path(from_path, "nonexist", got, sizeof got) == 0,
          "resolve_module_path misses nonexist");
    /* 含分隔符的模块名应被拒 */
    CHECK(resolve_module_path(from_path, "a/b", got, sizeof got) == 0,
          "resolve_module_path rejects slash in name");
  ...
  /* ---- workspace_get_unit + sem_resolve_export ---- */
  {
    ...
    if (wu.unit && wu.doc) {
      ...
      CHECK(sem_resolve_export(wu.unit, wu.doc, "SECRET", &xr) == 1,
            "sem_resolve_export finds SECRET");
    ...
    if (res && cJSON_IsArray(res)) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (lbl && lbl->valuestring) {
          if (strcmp(lbl->valuestring, "add") == 0)
            has_add = 1;
          if (strcmp(lbl->valuestring, "SECRET") == 0)
            has_secret = 1;
          if (strcmp(lbl->valuestring, "private_helper") == 0)
            has_private = 1;
        }
      }
    }
  ...
  {
    const char *a2_text = "import { ghost } from \"no_such_module\"\n"
    ...
    CHECK(degrade_ok,
          "definition on ghost (no module) degrades gracefully (no crash, no bogus target)");
    cJSON_Delete(r);
  }
  ...
  printf("Testing: declare-from linkage (C binding module)...\n");
  {
    ...
    const char *e_text = "declare from \"sdl\" {\n"
                         "    int Init(int flags);\n"
    ...
    if (res) {
      ...
      CHECK(line && line->valueint == 1,
            "declare-from: definition line == 1 (Init in declare block)");
    }
    cJSON_Delete(r);
    ...
    res = r ? cJSON_GetObjectItemCaseSensitive(r, "result") : NULL;
    CHECK(res != NULL && !cJSON_IsNull(res),
          "declare-from: definition on INIT_VIDEO returns Location");
    if (res) {
      cJSON *uri = cJSON_GetObjectItemCaseSensitive(res, "uri");
      CHECK(uri && strstr(uri->valuestring, "e.spt") != NULL,
  ...
  if (failed == 0) {
    printf("=== TestCrossImport: ALL PASS ===\n");
    return 0;
  }
