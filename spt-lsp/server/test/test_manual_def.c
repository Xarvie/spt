    ...
    if (resp) {
      ...
      if (result && cJSON_IsObject(result)) {
        const char *resp_uri = cJSON_GetObjectItem(result, "uri") ?
          cJSON_GetObjectItem(result, "uri")->valuestring : NULL;
    ...
    if (resp) {
      ...
      if (result && cJSON_IsObject(result)) {
        ...
        if (changes && cJSON_IsObject(changes)) {
          ...
          while (child) {
            n_files++;
            if (strstr(child->string, "utils.spt")) has_utils = 1;
            if (strstr(child->string, "services.spt")) has_svc = 1;
    ...
    if (resp) {
      ...
      if (result && cJSON_IsArray(result)) {
        ...
        for (int i = 0; i < n; i++) {
          ...
          if (u && strstr(u->valuestring, "utils.spt")) has_utils = 1;
          if (u && strstr(u->valuestring, "services.spt")) has_svc = 1;
        }
      ...
      if (result && cJSON_IsArray(result) && cJSON_GetArraySize(result) > 0) {
        ...
        if (nt && cJSON_IsString(nt)) {
          ...
          for (int i = 0; nt->valuestring[i] && i < 200; i++) {
            if (nt->valuestring[i] == '\t') printf("\\t");
            else if (nt->valuestring[i] == '\n') printf("\\n\n  ");
            else putchar(nt->valuestring[i]);
          }
          printf("\n");
          ...
          if (p) {
            ...
            int has_spaces_before = (p >= nt->valuestring + 4 && p[-1] == ' ' && p[-2] == ' ' && p[-3] == ' ' && p[-4] == ' ');
