...
*/
static int new_localvar(LexState *ls, TString *name) { return new_varkind(ls, name, VDKREG); }
...
static Upvaldesc *allocupvalue(FuncState *fs) {
  ...
  luaM_growvector(fs->ls->L, f->upvalues, fs->nups, f->sizeupvalues, Upvaldesc, MAXUPVAL,
                  "upvalues");
...
static void singlevar(LexState *ls, expdesc *var) { buildvar(ls, str_checkname(ls), var); }
...
static l_noret jumpscopeerror(LexState *ls, Labeldesc *gt) {
  ...
  luaK_semerror(ls, "<goto %s> at line %d jumps into the scope of '%s'", getstr(gt->name), gt->line,
