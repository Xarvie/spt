...
/*
** Return a nonzero value if integer 'k' is a valid 0-based array index
** in [0, limit-1].  The returned value is k+1 (so 0 means "not valid").
** All code using the result tests '> 0' and subtracts 1 to get the slot.
*/
static unsigned checkrange (lua_Integer k, unsigned limit) {
  return (l_castS2U(k) < limit) ? cast_uint(k) + 1u : 0;
}
...
int luaH_next (lua_State *L, Table *t, StkId key) {
  ...
  for (; i < asize; i++) {  /* try first array part */
    ...
    if (!tagisempty(tag)) {  /* a non-empty entry? */
      setivalue(s2v(key), cast_int(i));  /* report 0-based key */
      farr2val(t, i, tag, s2v(key + 1));
      return 1;
    }
  }
...
static void reinsertOldSlice (Table *t, unsigned oldasize,
  ...
  for (i = newasize; i < oldasize; i++) {  /* traverse vanishing slice */
    ...
    if (!tagisempty(tag)) {  /* a non-empty entry? */
      TValue key, aux;
      setivalue(&key, l_castU2S(i));  /* 0-based key */
...
static void luaH_newkey (lua_State *L, Table *t, const TValue *key,
  ...
  if (t->mode == TABLE_ARRAY) {
    if (ttisinteger(key))
      luaG_runerror(L, "list index out of range");
    else
...
void luaH_finishset (lua_State *L, Table *t, const TValue *key,
  ...
  if (hres == HNOTFOUND) {
    ...
    if (t->mode == TABLE_ARRAY) {
      if (ttisinteger(key))
        luaG_runerror(L, "list index out of range: %I (size: %d)",
                      (LUAI_UACINT)ivalue(key), t->asize);
      else
        luaG_runerror(L, "list indices must be integers");
    }
    /* ====== */
...
static lua_Unsigned hash_search (lua_State *L, Table *t, unsigned asize) {
  /* caller ensures key 'asize' is present in the hash part */
  lua_Unsigned i = asize;  /* known-present key; probe for absent keys above */
...
** such that t[i] is present and t[i+1] is absent, or 0 if t[0] is absent,
...
lua_Unsigned luaH_getn (lua_State *L, Table *t) {
  ...
  if (isdummy(t) || hashkeyisempty(t, asize))
    return asize;  /* key 'asize' is absent; length = asize */
  else  /* key 'asize' is also present */
    return hash_search(L, t, asize) + 1;
}
