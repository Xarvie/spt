...
LUALIB_API int luaL_loadbufferx(lua_State *L, const char *buff, size_t size, const char *name,
                                const char *mode) {
  LoadS ls;
  ls.s = buff;
  ls.size = size;
...
LUALIB_API void luaL_checkversion_(lua_State *L, lua_Number ver, size_t sz) {
  ...
  if (sz != LUAL_NUMSIZES) /* check numeric types */
    ...
    luaL_error(L, "version mismatch: app. needs %f, Lua core provides %f", (LUAI_UACNUMBER)ver,
               (LUAI_UACNUMBER)v);
}
