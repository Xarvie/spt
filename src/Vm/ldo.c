...
extern Proto *astY_compileFunction(lua_State *L, FuncState *parent_fs,
                                      Dyndata *dyd, struct AstNode *funcNode,
                                      const char *name);
...
static void f_parser (lua_State *L, void *ud) {
  ...
  if (c == LUA_SIGNATURE[0]) {
    // 恢复 Lua 原生逻辑：计算 fixed 参数
    int fixed = 0;
    const char *mode = p->mode ? p->mode : "bt"; // 确保 mode 不为空
    ...
    // 2. 处理文本代码
    // [修复开始]：给 mode 设置默认值 "bt"，防止 checkmode 崩溃
    const char *mode = p->mode ? p->mode : "bt";
    checkmode(L, mode, "text");
    // [修复结束]

    // --- 步骤 A: 读取 ZIO 流到 Buffer 中 ---
    luaZ_resetbuffer(&p->buff);

// 定义一个简单的宏来处理 buffer 扩容 (仅在此函数内部使用)
#define MY_ADD_CHAR(B, ch) \
      if ((B)->n >= (B)->buffsize) { \
        size_t newsize = (B)->buffsize > 0 ? (B)->buffsize * 2 : 32; \
    ...
    // 添加字符串结束符 \0
    MY_ADD_CHAR(&p->buff, '\0');

#undef MY_ADD_CHAR // 用完取消定义
