...
LUA_API lua_Unsigned lua_rawlen(lua_State *L, int idx) {
  ...
  case LUA_VARRAY:
    return cast(lua_Unsigned, avalue(o)->loglen); /* loglen, not asize */
  case LUA_VTABLE: {
    lua_Unsigned res;
    lua_lock(L);
...
l_sinline Table *gettable(lua_State *L, int idx) {
  ...
  api_check(L, ttistable(t) || ttisarray(t), "table or array expected");
  return gco2t(gcvalue(t));
}
...
LUA_API void lua_createarray(lua_State *L, int narray) {
  ...
  lua_lock(L);
  t = luaH_newarray(L);
  setavalue2s(L, L->top.p, t);
  api_incr_top(L);
  if (narray > 0)
    luaH_resizearray(L, t, cast_uint(narray));
  /* Note: loglen stays 0 after createarray; caller sets it via setlen
     or by writing elements through seti which triggers finishset append */
  luaC_checkGC(L);
  lua_unlock(L);
}
...
/*
** Resize the physical capacity of an array.
** Cannot shrink below the current logical length.
*/
LUA_API void lua_arrayresize(lua_State *L, int idx, lua_Integer newsize) {
  ...
  t = avalue(o);
  api_check(L, cast_uint(newsize) >= t->loglen, "cannot shrink capacity below logical length");
  luaH_resizearray(L, t, cast_uint(newsize));
  lua_unlock(L);
}

/*
** Get the logical length of an array (loglen, not physical capacity).
*/
LUA_API lua_Integer lua_arraylen(lua_State *L, int idx) {
  ...
  o = index2value(L, idx);
  api_check(L, ttisarray(o), "array expected");
  len = cast(lua_Integer, avalue(o)->loglen); /* logical length */
  lua_unlock(L);
  return len;
}

/*
** Set the logical length of an array.
** newlen must be in [0, asize]. If shrinking, truncated slots are
** cleared to LUA_VEMPTY for GC safety.
*/
LUA_API void lua_arraysetlen(lua_State *L, int idx, lua_Integer newlen) {
  ...
  api_check(L, ttisarray(o), "array expected");
  t = avalue(o);
  api_check(L, newlen >= 0 && cast_uint(newlen) <= t->asize, "length out of capacity");
