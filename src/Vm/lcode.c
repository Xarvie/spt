...
static void savelineinfo(FuncState *fs, Proto *f, int line) {
  ...
  if (abs(linedif) >= LIMLINEDIFF || fs->iwthabs++ >= MAXIWTHABS) {
    luaM_growvector(fs->ls->L, f->abslineinfo, fs->nabslineinfo, f->sizeabslineinfo, AbsLineInfo,
                    INT_MAX, "lines");
...
void luaK_indexed(FuncState *fs, expdesc *t, expdesc *k) {
  ...
  lua_assert(!hasjumps(t) &&
             (t->k == VLOCAL || t->k == VVARGVAR || t->k == VNONRELOC || t->k == VUPVAL));
...
void luaK_setlistsize(FuncState *fs, int pc, int ra, int asize) {
  ...
  int rc = asize % (MAXARG_vC + 1);    /* lower bits of array size */
  int k = (extra > 0);                 /* true iff needs extra argument */
  *inst = CREATE_vABCk(OP_NEWLIST, ra, 0, rc, k);
  *(inst + 1) = CREATE_Ax(OP_EXTRAARG, extra);
}
