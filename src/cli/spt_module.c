...
**
** 实现说明：不再就地修改 Lua 字符串（TString 不可变，原写法是 UB）。
** 改为把展开后的路径串复制到 malloc 缓冲，再在副本上做分隔切分。
*/
static char *find_spt_file(lua_State *L, const char *modname) {
  ...
  const char *expanded = lua_tostring(L, -1);
  size_t expanded_len = strlen(expanded);
  /* 复制到 malloc 缓冲，后续在副本上做 strtok 风格的切分 */
  char *copy = (char *)malloc(expanded_len + 1);
  char *result = NULL;
  if (!copy) {
    lua_pop(L, 1);
    return NULL;
  }
  memcpy(copy, expanded, expanded_len + 1);

  for (char *p = copy; p < copy + expanded_len;) {
    /* find next ';' separator */
    char *sep = (char *)memchr(p, ';', (size_t)(copy + expanded_len - p));
    if (!sep)
      sep = copy + expanded_len;
    *sep = '\0';
