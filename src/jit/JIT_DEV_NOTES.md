...
链式 `m[i][j]` 仍另案：DCE 修复后它能成环了，但暴露出另一个 miscompile——常量外层下标
`m[0][j]` 里 `m[0]` 是个**产出数组的常量下标 GETI**、被 LICM 提升后结果错（变量下标 `m[i][j]` 正确）。
已再次回退链式递归 eval（干净 abort，0.9×）；显式 `row=m[i]` 仍可用。待修该 ARR-GETI 提升 miscompile。
