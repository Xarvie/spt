...
#### 仍未解决：嵌套循环里"数组载入值相乘"不成环（pre-existing）

系统性缩小后定位到一个**与本轮改动无关的既有问题**（关/开提升都复现）：
- `s += a[j] + b[j]`（两次载入、同下标）：**成环**，entries=外层数。
- `s += a[j] + b[k]`（两次载入、异下标，k 不变）：**成环**。
- 标量 `s += i*i`（纯 MUL）：**成环**，entries=1，12.5×。
- `s += b[k]`（单个变量不变下标）：**成环**，2.0×。
- 但 `s += a[j]*a[j]` 和 `s += a[j]*b[k]`（**数组载入值相乘**）：**不成环**，
  entries≈总迭代数，exits=0/guard_fail=0，0.7×（比解释器慢）。

即：不是 MUL 本身（标量 MUL 成环），不是数组载入本身（求和成环），而是**嵌套循环里把两个
数组载入的值相乘**这个特定组合会破坏内部成环。trace 里 LOOP 指令在、向后跳转（back-edge）也在
（与 ADD 版本反汇编结构一致，都跳同一个 loop_label），但运行期就是逐迭代进入、且没有 guard
退出（exits=0）。机制仍未查清，是当前最高价值的待解性能问题（解开同时能救活点积和链式二维）。
属于和链式 `m[i][j]` 同一类的"有合法 LOOP + back-edge 却不在内部成环"之谜。

**本轮更深一层的排查（仍未定位根因，但缩小了范围）**：
- **编译出来的循环体和 prologue 与 ADD 版本逐指令同构**：同样的寄存器分配（rsi=s、rdi=count、
  r15=j、r8=step）、同样的 5 个 guard（j>=0 / j<LEN / INT 类型 / 计数）、同样的无条件
  back-edge `jmp 0x96`，连中间结果都同样落到 spill 再回读。唯一区别就是值合并那条指令
  `imul rax, rcx` vs `add rax, rcx`（同寄存器、同 flag 足迹）。标量 `i*i`（imul、操作数在寄存器）
  照常成环，所以不是 imul 指令本身，而是"imul + 溢出到 spill 的数组载入操作数"这个组合。
- **abort 行为与 ADD 完全相同**：两者都在 start_pc=17 录制、5 条指令后于 op=74（pc=27）abort
  共 8 次（到 MAX_ABORTS）。所以 abort/黑名单不是区别所在。
- **规模相关**：外层很小（如 outer=50）时 MUL **也成环**（entries≈outer）；外层大到 ~100k 以上时
  变成逐迭代（entries≈(inner-1)×outer，即每次进 trace 只跑约 1 次内层迭代就退出）。HOT 阈值
  4/8/16/40/100 在大规模下都逐迭代。即同一份编译代码在小规模成环、大规模不成环。
- `trace_exits`、`trace_guard_fail` 这两个统计**从未被自增**（代码里只在打印处出现），所以
  "exits=0 / guard_fail=0"不携带信息，别被误导。

结论：问题不在发射出来的循环体（它和能成环的 ADD 同构），而在**录制/再进入的动态行为**——大规模下
trace 每跑约 1 次内层迭代就回到解释器、再被重新派发进来。下一步该上的工具是给退出加真正的
side-exit 计数与 exit PC 日志，定位"大规模下从哪个 guard、resume 到哪个 PC"。

完整回归：16 kernel + 230 ctest + 全量差分 227/228 全绿（那个 1 是 b4_yield_return 打印不确定
堆地址，JIT 关掉也不一致，非 bug）。
