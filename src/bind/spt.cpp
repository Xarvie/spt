...
spt::Value cFunctionTrampoline(spt::VM *vm, spt::Closure *self, int argc, spt::Value *argv) {

  spt_State *S = static_cast<spt_State *>(vm->getUserData());
  if (!S) {
    vm->throwError(spt::Value::object(vm->allocateString("Internal error: no state")));
    return spt::Value::nil();
  }

  if (!self->isNative() || self->upvalueCount < 1) {
    vm->throwError(spt::Value::object(vm->allocateString("Invalid C function closure")));
    return spt::Value::nil();
  }

  spt::Value funcVal = self->getNativeUpvalue(0);
  if (!funcVal.isInt()) {
    vm->throwError(spt::Value::object(vm->allocateString("Invalid C function pointer")));
    return spt::Value::nil();
  }
  ...
  int nResults = cfunc(S);

  if (nResults == 0) {
    fiber->stackTop = oldTop;
    return spt::Value::nil();
  } else if (nResults == 1) {
    spt::Value result = fiber->pop();
    fiber->stackTop = oldTop;
    return result;
  } else {
    ...
    return results.empty() ? spt::Value::nil() : results[0];
  }
}
...
SPT_API void spt_mapkeys(spt_State *S, int idx) {
  ...
  spt::MapObject *map = static_cast<spt::MapObject *>(v.asGC());
  spt::ListObject *keys = S->vm->allocateList(static_cast<int>(map->entries.size()));
...
SPT_API void spt_mapvalues(spt_State *S, int idx) {
  ...
  spt::MapObject *map = static_cast<spt::MapObject *>(v.asGC());
  spt::ListObject *values = S->vm->allocateList(static_cast<int>(map->entries.size()));
...
SPT_API void spt_defmodule(spt_State *S, const char *name, const spt_Reg *funcs) {
  ...
  spt::ModuleManager *mm = S->vm->moduleManager();
  if (mm) {
    std::vector<spt::MethodEntry> entries;
    for (const auto &f : funcList) {

      spt::MethodEntry entry;
      entry.name = f.name;
      entry.arity = f.arity;

      entry.fn = [](spt::VM *vm, spt::Closure *self, int argc, spt::Value *argv) -> spt::Value {
        return spt::Value::nil();
      };
      entries.push_back(entry);
    }
    entries.push_back({nullptr, nullptr, 0});
  }
}
...
SPT_API void spt_error(spt_State *S, const char *fmt, ...) {
  ...
  setError(S, msg.c_str());

  spt::StringObject *errStr = S->vm->allocateString(msg);
  S->vm->throwError(spt::Value::object(errStr));
}
...
SPT_API void spt_gcstats(spt_State *S, size_t *totalCollections, size_t *bytesAllocated,
                         size_t *objectCount, size_t *peakBytes) {
  if (!S)
    return;

  const spt::GCStats &stats = S->vm->gc().stats();

  if (totalCollections)
    *totalCollections = stats.totalCollections;
  if (bytesAllocated)
    *bytesAllocated = S->vm->gc().bytesAllocated();
  if (objectCount)
    *objectCount = S->vm->gc().objectCount();
  if (peakBytes)
    *peakBytes = stats.peakBytesAllocated;
}
